...
const SubmissionList: React.FC<SubmissionListProps> = ({
  ...
  useEffect(() => {
    ...
    if (showPendingOnly) {
      ...
      if (statusFilter) {
        currentFilters.status_filter = statusFilter;
      } else {
        delete currentFilters.status_filter;
      }
      fetchSubmissions(currentFilters);
    }
  }, [showPendingOnly, statusFilter]);

  const handleStatusFilterChange = (status: SubmissionStatus | '') => {
    console.log('🔍 Status filter changing to:', status);
  ...
  return (
    <div className="space-y-4">
      {/* Filters - Always render when not showing pending only */}
      {!showPendingOnly && (
        <div className="flex items-center space-x-4" key="filter-container">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              handleStatusFilterChange(e.target.value as SubmissionStatus | '');
            }}
