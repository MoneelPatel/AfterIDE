...
# Tests all implemented functionality for Phase 1B

echo "🧪 Phase 1B Testing Suite"
echo "=========================="
...
# Use a quick timeout to avoid hanging - frontend is on port 3000
FRONTEND_STATUS=$(curl -s --max-time 5 -o /dev/null -w '%{http_code}' http://localhost:3000 2>/dev/null || echo "000")
if [ "$FRONTEND_STATUS" = "200" ]; then
    echo -e "${GREEN}✅ Frontend is accessible (Status: $FRONTEND_STATUS)${NC}"
    ((TESTS_PASSED++))
elif [ "$FRONTEND_STATUS" = "000" ]; then
    echo -e "${YELLOW}⚠️  Frontend connection timeout or not running${NC}"
    ((TESTS_FAILED++))
else
...
if [ $TESTS_FAILED -eq 0 ]; then
    echo -e "\n${GREEN}🎉 All Phase 1B tests passed!${NC}"
    exit 0
else
